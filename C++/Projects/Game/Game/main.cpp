...
public:
	...
	void update(float time)			//функция "оживления" класса
	{
		...
		//if (CurrentFrame > 12) 
			//CurrentFrame -= 12;
		//if (dx > 0) 
			//sprite.setTextureRect(sf::IntRect(32 * int(CurrentFrame), 132, 28, 28));
		//else if (dx < 0) 
			//sprite.setTextureRect(sf::IntRect(32 * int(CurrentFrame) + 32 - 8, 132, -28, 28));
	}
};
...
public:
	...
	Bullet(sf::Image &image, sf::String Name, Level &lvl, float X, float Y, int W, int H, float tX, float tY) :Entity(image, Name, X, Y, W, H)			//всё так же, только взяли в конце состояние игрока (int dir)
	{
...
bool startGame(sf::RenderWindow &window, int &numberLevel, bool &showMainMenu)
{
	...
	Player p(heroImage, "Player1", lvl, player.rect.left, player.rect.top, 40, 30);			//передаем координаты прямоугольника player из карты в координаты нашего игрока 40 30
	...
	while (window.isOpen()) {
		...
		while (window.pollEvent(event)) {
			...
			if (sf::Keyboard::isKeyPressed(sf::Keyboard::BackSpace)) {			//если таб, то перезагружаем игру
				showMainMenu = true;
				return true;
			}
			...
			if (event.type == sf::Event::MouseButtonPressed)		//если нажата клавиша мыши
				if (event.key.code == sf::Mouse::Left) {			//а именно левая, то стреляем и тут же снимаем координаты мыши в этом месте
					...
					entities.push_back(new Bullet(BulletImage, "Bullet", lvl, p.x + (p.w / 2), p.y, 16, 16, pos.x, pos.y));				//и передаем в конструктор пули. создается пуля
		...
		for (it = entities.begin(); it != entities.end(); it++) {			//вновь проходимся по элементам списка
			...
			for (it2 = entities.begin(); it2 != entities.end(); it2++) {
				if ((*it)->getRect() != (*it2)->getRect()) {			//при этом это должны быть разные прямоугольники
					...
					if (((*it)->getRect().intersects((*it2)->getRect())) & ((*it)->name == "EasyEnemy") & ((*it2)->name == "Bullet")) {			//если столкнулись два объекта враг и пуля
						(*it)->life = false;			//убиваем врага
...
int main()
{
	//sf::RenderWindow window(sf::VideoMode(1280, 720), "Game", sf::Style::Default);			//инициализация окна
