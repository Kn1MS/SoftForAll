...
void gameMode(std::string &word, std::string hangman[])									//game function
{
	...
	while (process) {
		...
		if ((var == '0') & (buff.empty())) {
			...
			std::cout << "You entered not only English letters! Try again.\n";	// -32=-1, -64=-33, -72, -88
			std::system("pause");												//1025 + 1040-1071, 1072-1103 + 1105 russian unicode
			std::system("cls");
			outWord(word, check, buk, hangman);
		}
					...
					for (unsigned int i = 0; i < word.size(); i++)
						if ((i == 0) | (i == word.size() - 1)) {
							if ((((char)word[i] < 65) | ((char)word[i] > 90)) & (((char)word[i] < 97) | ((char)word[i] > 122))) {
								std::cout << "Word consists not only English letters! Fix it in input file.\n";
								worcdcheck = false;
								break;
							}
						}
